...
CHRISTMAS_EVENTS = [
    {
        "id": "santa_claus",
        "text": "🎅 بابانوئل در شهر است!\nاولین کسی که با 🎅 جواب بده، مقدار زیادی میوپوینت می‌گیرد!",
        "answer": "🎅",
        "reward": {"type": "points", "amount": 200},
    },
    {
        "id": "snowball_fight",
        "text": "☃️ جنگ گلوله برفی گربه‌ها!\nاولین کسی که با ☃️ جواب بده، میوپوینت می‌گیرد!",
        "answer": "☃️",
        "reward": {"type": "points", "amount": 80},
    },
    {
        "id": "gift_exchange",
        "text": "🎁 زمان تبادل هدایا!\nاولین کسی که با 🎁 جواب بده، میوپوینت می‌گیرد!",
        "answer": "🎁",
    ...
    {
        "id": "mistletoe_magic",
        "text": "💋 زیر داروش‌سبز جادویی!\nاولین کسی که با 💋 جواب بده، میوپوینت خفن می‌گیرد!",
        "answer": "💋",
...
REGULAR_EVENTS = [
    {
        "id": "homeless_cat",
        "text": "📢 رویداد روزانه:\nیک گربه‌ی بی‌خانمان دم گروه پرسه می‌زنه!\nاولین کسی که فقط با ایموجی 🏠 جواب بده، میوپوینت می‌گیرد.",
        "answer": "🏠",
...
async def cmd_clan_leave(message: types.Message):
    ...
    if clan_info["leader_id"] == user_db_id:
        ...
        success = delete_clan(clan_info["id"])
        if success:
            await message.reply(f"🗑️ کلن **{clan_info['name']}** منحل شد زیرا رهبر آن را ترک کرد.")
        else:
    ...
    if success:
        await message.reply(f"👋 شما با موفقیت از کلن **{clan_info['name']}** خارج شدید.")
    else:
...
async def cmd_clan_list(message: types.Message):
    ...
    for i, clan in enumerate(clans, 1):
        ...
        text += (
            ...
            f"   👑 رهبر: {clan['leader_username'] or 'نامشخص'}\n"
